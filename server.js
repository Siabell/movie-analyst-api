...
app.get('/pending', async function (req, res) {
  try {
    const rows = await pool.query(
      'select m.title, m.release_year, m.score, r.name as reviewer, p.name as publication from movies m, reviewers r, publications p where r.publication=p.name and m.reviewer=r.name and m.release_year>=2017'
    )
    res.json(rows)
  } catch (err) {
    console.error('API Error:', err)
